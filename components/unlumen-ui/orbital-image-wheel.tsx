...
function getSnapProgressForIndex(
  ...
  for (let cycle = minCycle; cycle <= maxCycle; cycle += 1) {
    ...
    if (distance < minDistance) { minDistance = distance; nearest = progress; }
  }
  if (!Number.isFinite(minDistance)) return clamp((safeIndex / total - 0.25) / turns, 0, 1);
  return nearest;
}
...
export function OrbitalImageWheel({
  ...
  const boundedSubtitleDirection = subtitleDirection === "bottom" ? "bottom" : "top";
  ...
  const activeTitleTrackIndex = Math.max(0, Math.min(activeIndex, titleTrackLabels.length - 1));
  ...
  const handleTitleClick = useCallback(
    (index: number) => {
      ...
      const targetProgress = getSnapProgressForIndex(index, images.length, boundedTurns, currentProgress);
      ...
      titleClickTweenRef.current = gsap.to(proxy, {
        ...
        ease: "power3.out",
        overwrite: true,
        onUpdate: () => { trigger.scroll(proxy.scroll); },
  ...
  useEffect(() => {
    ...
    const context = gsap.context(() => {
      ...
      const applyState = (rawProgress: number) => {
        ...
        const focusedIndex = getFocusedImageIndexWithHysteresis(p, cards.length, boundedTurns, previousActive);
        cards.forEach((card, index) => {
          ...
          const currentBrightness = boundedDim + (1 - darkIntensity) * (peakBrightness - boundedDim);
          const currentSaturation = boundedMinSaturation + (1 - saturationIntensity) * (100 - boundedMinSaturation);
          ...
          gsap.set(card, {
            x, y, xPercent: -50, yPercent: -50, z: depth,
            rotate: tilt, scale: currentScale,
    ...
    scrollContainerRef, images, radius, boundedTurns, boundedBlur, boundedDim,
    boundedBrightnessBoost, boundedDarknessStrength, boundedMinSaturation,
    boundedSaturationStrength, boundedFocusSpread, boundedScaleEffect, boundedScrollSensitivity,
  ]);
  ...
  useLayoutEffect(() => {
    ...
    if (!titleTrackXToRef.current) {
      titleTrackXToRef.current = gsap.quickTo(track, "x", { duration: 0.62, ease: "power4.out", overwrite: true });
    }
    const firstTitle = track.querySelector<HTMLElement>(`[data-title-index="0"]`);
    const lastTitle = track.querySelector<HTMLElement>(`[data-title-index="${titleTrackLabels.length - 1}"]`);
    const activeTitle = track.querySelector<HTMLElement>(`[data-title-index="${activeTitleTrackIndex}"]`);
    if (!activeTitle || !firstTitle || !lastTitle) return;
    const viewportWidthPx = viewport.clientWidth;
    const startPad = Math.max(0, viewportWidthPx / 2 - firstTitle.offsetWidth / 2);
    const endPad = Math.max(0, viewportWidthPx / 2 - lastTitle.offsetWidth / 2);
    if (startSpacer) startSpacer.style.width = `${Math.round(startPad)}px`;
    if (endSpacer) endSpacer.style.width = `${Math.round(endPad)}px`;
    ...
    >
      <div ref={viewportRef} className="sticky top-0 h-screen w-full overflow-hidden">
        <div
          ...
          style={{ width: boundedWheelSize, height: boundedWheelSize, bottom: `-${boundedWheelSize * boundedCropRatio}px` }}
        >
            ...
            <div className="px-6 text-center">
              ...
              >
                <div ref={titleTrackRef} className="flex w-max items-center">
                  <span ref={titleStartSpacerRef} aria-hidden className="block h-px shrink-0" />
                  {titleTrackLabels.map((title, i) => (
                    <button
                      ...
                      aria-current={i === activeTitleTrackIndex ? "true" : undefined}
                      style={{
                        opacity: Math.abs(i - activeTitleTrackIndex) === 0 ? 1
                          : Math.abs(i - activeTitleTrackIndex) === 1 ? 0.58
                          : Math.abs(i - activeTitleTrackIndex) === 2 ? 0.32 : 0.16,
                        transform: Math.abs(i - activeTitleTrackIndex) === 0 ? "scale(1)" : "scale(0.96)",
                      }}
                      className={cn(
                  ...
                  <span ref={titleEndSpacerRef} aria-hidden className="block h-px shrink-0" />
                </div>
              </div>
            </div>
          </div>
